...
//-----------------------------------------------------------------------------
int PPZ8::Load(const TCHAR *filename, int bufnum)
{
...
//@		size2 = size2 / 2 - sizeof(PVIHEADER);		// PVI換算
		...
		if((pdst = XMS_FRAME_ADR[bufnum]
						= (uint8_t*)malloc(std::max(size, size2) * 2)) == NULL) {
			pfileio->Close();
			return _ERR_OUT_OF_MEMORY;			// メモリが確保できない
		}
		memset(pdst, 0, std::max(size, size2) * 2);
		
		if((psrc = psrc2 = (uint8_t*)malloc(std::max(size, size2))) == NULL) {
			pfileio->Close();
			return _ERR_OUT_OF_MEMORY;			// メモリが確保できない（テンポラリ）
		}
		memset(psrc, 0, std::max(size, size2));
