...
interface IPCMMUSICDRIVER : public IUnknown {
	virtual bool WINAPI init(const TCHAR *path) = 0;
	virtual int WINAPI music_load(const TCHAR *filename) = 0;
	virtual int WINAPI music_load2(const uint8_t *musdata, int size) = 0;
	...
	virtual bool WINAPI getlength(const TCHAR *filename, int *length, int *loop) = 0;
...
interface IFMPMD : public IPCMMUSICDRIVER {
	virtual bool WINAPI loadrhythmsample(const TCHAR *path) = 0;
	virtual bool WINAPI setpcmdir(const TCHAR **path) = 0;
	...
	virtual bool WINAPI getlength2(const TCHAR *filename, int *length, int *loop) = 0;
